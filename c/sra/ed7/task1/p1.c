...
int main(int argc,char*argv[])
{
	...
	if(fp1 == NULL) {  /*checking the file is exist or not */
		printf("file doesn't exist\n");
		return 0;
	}

	char cCh;
	while( (cCh = fgetc(fp1)) != EOF ){ /*reads all the characters from file */
		sStr1[iIndex1++]=cCh;
	}
	sStr1[iIndex1]='\0';
	fclose(fp1);
	...
	if(fp2 == NULL) {  /*checking the file is exist or not */
		printf("file doesn't exist\n");
		return 0;
	}

	char cCh2;
	while( (cCh2 = fgetc(fp2)) != EOF ) /*reads all the characters from file. */
		sStr2[iIndex2++]=cCh2;
	sStr2[iIndex2]='\0';
	fclose(fp2);
	int ivar1,ivar2;
