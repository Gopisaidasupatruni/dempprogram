...
do
    ...
    then
        ...
        # Extract the first character and make it uppercase using ^ and tr
        first_char_upper=$(echo "${filename:0:1}" | tr '[:lower:]' '[:upper:]')

        # Get the rest of the filename starting from the second character (-c2- equivalent)
        rest_of_filename=$(echo "${filename:1}" | tr '[:upper:]' '[:lower:]')
