...
void correct_foreground_pipeline(const char* cmd1, char* const* argv1, const char* cmd2, char* const* argv2) {
    ...
    close(pipefd[0]);
    waitpid(child1, &status, 0); // Wait for cmd1 to complete
    waitpid(child2, &status, 0); // Wait for cmd2 to complete
}

int main() {
    const char* cmd1 = "echo";
    char* const argv1[] = {"echo", "foo", NULL};

    const char* cmd2 = "wc";
    char* const argv2[] = {"wc", "-c", NULL};
